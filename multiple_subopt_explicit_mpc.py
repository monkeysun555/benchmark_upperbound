# This file is to calculate ratio between different lookahead horizon and optimal results
# There are three QoE weight settings
import os
import logging
import numpy as np
import live_player
...
def find_subopt(file_num, server_start_up, lookahead, curr_dir, temp_type):
	...
	elif temp_type == 2:			# Sensitive to latency
		ACTION_REWARD = 1.0 * CHUNK_SEG_RATIO	
		REBUF_PENALTY = 6.0		# for second
		SMOOTH_PENALTY = 1.0
		# LONG_DELAY_PENALTY_BASE = 1.2	# for second
	...
	r_table_pre.append([0.0, [-1], 0, 0.0, init_latency_shift, 0.0])	# accu reward, bitrate seq, and state, buffer shift, time shift
	pre_value_idx.append([0, init_round_latency, 0])
	...
	for seg_idx in range(TEST_DURATION):
		...
		c_pre_value_idx = pre_value_idx[:]
		c_r_table_pre = r_table_pre[:]
		for step in range(lookahead):
			...
			for idx in range(len(pre_value_idx)):
				...
				for bit_rate in range(len(BITRATE)):
					...
					server.clone_from_state(seg_idx + step, server_timing, bit_rate, ratio)	# Init from last saved state
							...
							if sync:
								...
								if [round_buffer_length, round_latency, bit_rate] not in curr_value_idx:
									if action_reward >= -20.0:
										...
										r_table_curr.append([action_reward, temp_seq, temp_state, temp_buffer_shift, temp_latency_shift, temp_playing_time])
								else:
									...
									r_idx = curr_value_idx.index([round_buffer_length, round_latency, bit_rate])
									if action_reward >= r_table_curr[r_idx][0]:
										temp_state = player.get_state()
										r_table_curr[r_idx] = [action_reward, temp_seq, temp_state, temp_buffer_shift, temp_latency_shift, temp_playing_time]
		...
		# timing = value[1] * TIMING_BIN * MS_IN_S
		latency = c_pre_value_idx[0][1] * LATENCY_BIN * MS_IN_S
		last_bit_rate = c_pre_value_idx[0][2]
		element = c_r_table_pre[0]
		...
		# print "latency: ", latency + latency_shift
		# print "Server timing: ", server_timing
		server.clone_from_state(seg_idx, server_timing, sub_bit_rate, ratio)	# Init from last saved state
		player.clone_from_state(player_timing, buffer_length + buffer_shift, state, playing_time)
		take_action = 1
		temp_latency = 0.0
		missing_count = 0.0
		...
		while True:
			...
			chunk_number = download_chunk_end_idx - download_chunk_idx + 1
			assert chunk_number == 1
			assert download_seg_idx == seg_idx
			server_wait_time = 0.0
			sync = 0
			missing_count = 0
			real_chunk_size, download_duration, freezing, time_out, player_state = player.fetch(download_chunk_size, 
			...
			if temp_type == 1:
				reward = ACTION_REWARD * log_bit_rate * chunk_number \
						- REBUF_PENALTY * freezing / MS_IN_S \
						- SMOOTH_PENALTY * np.abs(log_bit_rate - log_last_bit_rate) \
						- LONG_DELAY_PENALTY*(LONG_DELAY_PENALTY_BASE**(temp_latency/ MS_IN_S)-1) * chunk_number \
						- MISSING_PENALTY * missing_count
			else:
				reward = ACTION_REWARD * log_bit_rate * chunk_number \
						- REBUF_PENALTY * freezing / MS_IN_S \
...
def main():
	...
	for t in TYPES:
		curr_dir = SUMMARY_DIR + 'type_' + str(t) + '/'
