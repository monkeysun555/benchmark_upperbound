...
def find_upper(file_num, server_start_up, curr_dir, temp_type, trace_names, trace_times, trace_bws):
...
def main():
    ...
    curr_dir = SUMMARY_DIR + '/'
    res_que = []
    ## CHANGE HERE## TON REVISION
    buffer_len = 2      # 2 AND 3
    qoe_type = 2        # DONT CHANGE FOR MASSIVE
    # For each t/bf combination, run 10 processes of different network env
    processes = []
    for file_num in range(num_process):
        processes.append(Process(target=find_upper, args=(file_num, buffer_len, curr_dir, qoe_type)))
        processes[-1].start()
